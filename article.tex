  ...
  \begin{minipage}{0.5\textwidth}
    \begin{flushleft}
  	...
  	\begin{prooftree}
    	...
    	\RightLabel{$\vdash {\bf K}_I$}
    	\UnaryInfC{${\bf K}(\Gamma), \Delta \vdash {\bf K}A$}
  \end{prooftree}
    \end{flushleft}
  \end{minipage}
  \begin{minipage}{0.5\textwidth}
    \begin{flushright}
  	...
  	\begin{prooftree}
    \AxiomC{${\bf K}(\Gamma), \Delta \vdash {\bf K} A$}
    \AxiomC{${\bf K}(\Gamma), {\bf K} A, \Delta \vdash B$}
    \BinaryInfC{${\bf K}(\Gamma), \Delta \vdash B$}
  	\end{prooftree}
    \end{flushright}
  \end{minipage}
  \end{defin}
...
\AxiomC{}
\RightLabel{$ax_1$}
\UnaryInfC{$\Gamma | \Delta, x : A \vdash A$}
\end{prooftree}

\begin{prooftree}
\AxiomC{$\Gamma | \Delta, x : A \vdash M : B$}
\RightLabel{$\to_i$}
\UnaryInfC{$\Gamma | \Delta \vdash (\lambda x. M) : A \to B$}
\end{prooftree}

\begin{prooftree}
\AxiomC{$\Gamma | \Delta \vdash M : A$}
\AxiomC{$\Gamma | \Delta \vdash N : B$}
\RightLabel{$\times_i$}
\BinaryInfC{$\Gamma | \Delta \vdash \langle M, N \rangle : A \times B$}
\end{prooftree}

\begin{prooftree}
\AxiomC{$\Gamma | \Delta \vdash M : A$}
\RightLabel{${\bf K}_I$}
\UnaryInfC{$\Gamma | \Delta \vdash {\bf pure \: } M : {\bf K} A$}
\end{prooftree}
\end{minipage}%
    \hfill
    \begin{minipage}{0.45\textwidth}
...
Here are some examples of derivation trees.
  ...
  \begin{prooftree}
  \AxiomC{$\vdash \lambda f. \lambda x. {\bf let \: pure \: } g = f {\bf \: in \:} {\bf let \: pure \: }$}
  \end{prooftree}
...
\end{proof}
  ...
  \bibitem{DePaiva} de Paiva V., Ritter E., Bellin, G. Extended Curry-Howard correspondence for a basic constructive modal logic. Methods for Modalities Conference (M4M-2); 2001 November 29-30; Amsterdam; the Netherlands.
\end{thebibliography}
