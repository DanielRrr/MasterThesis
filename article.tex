...
Proof is similar to \cite{Baren} \cite{Baren2}. We build the following diagram, which commutes by lemmas 5 and 7.
...
\begin{proof}
  ...
  Unfold $M \twoheadrightarrow_{\beta} N$ as the sequence of one-step reduction and apply strip lemma on the every step.
\end{proof}
...
\section{Monadic extention}

In modern Haskell \verb"Monad" class is an extention of the \verb"Applicative" class,
where monadic binding (\verb"(>>=)") is the single basic class method:
...
\begin{defin} ``Monadic'' intuitionistic epistemic logic IEL$^{-}$
...
Categorically \verb"Monad" class is a Kleisli triple on monoidal category, or, which is the same, strong monoidal monad:

\begin{defin} Strong monoidal monad


\end{defin}

\begin{defin} Monadic $\lambda_{{\bf K}}$

  Monadic $\lambda_{{\bf K}}$ is a $\lambda_{{\bf K}}$ with the next additional typing rule:

  \begin{prooftree}
    \AxiomC{$\Gamma \vdash M : {\bf KK}A$}
    \RightLabel{${\bf K}^2$--e}
    \UnaryInfC{$\Gamma \vdash {\bf join \:} M : {\bf K}A$}
  \end{prooftree}
\end{defin}
