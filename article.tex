...
\end{prooftree}
  ...
  15) ${\bf pure \:} ({\bf let \: pure \:} \underline{\quad} = \underline{\quad} {\: \bf in \:} N) \rightarrow_{\eta} { \bf pure \:} ({\bf pure \:} N)$.
  \end{defin}
...
By $\text{let}_{\bf K}$, $\Gamma \uparrow FV(\vec{M}) \vdash {\bf let \: pure \:} \vec{x} = \vec{M} {\: \bf in \:} N : {\bf K} B$
  ...
  i) Let $\Gamma \vdash M : A$ and $M \twoheadrightarrow_{\beta} N$, then $\Gamma \vdash N : A$

  ii) Let $\Gamma \vdash M : A$ and $M \twoheadrightarrow_{\eta} N$, then $\Gamma \vdash N : A$
  \end{theorem}
...
We consider only modal $\beta$-reduction rules. The general statement for $\twoheadrightarrow_{\beta}$ follows from transitivity of multi-step $\beta$-reduction.
  ...
  i) For multistep $\beta$-reduction:
...
ii) For multistep $\eta$-reduction:

\vspace{\baselineskip}

  1) Let $\vdash {\bf let \: pure \:} \underline{\quad} = \underline{\quad} {\: \bf in \:} N : {\bf K} A$.

  Then by generation $\vdash N : A$, so $\vdash {\bf pure \:} N : {\bf K} A$ by ${\bf K}_I$.

  \vspace{\baselineskip}

  2) Let $\Gamma \vdash {\bf let \: pure \:} x = M {\: \bf in \: } x : {\bf K} A$.

  By generation $\Gamma \vdash M : {\bf K} A$ and $x : A \vdash x : A$, hence $\Gamma \vdash M : {\bf K} A$.

  \vspace{\baselineskip}

  3) Let $\Gamma \vdash {\bf pure \:} (\lambda x. f x) : {\bf K}(A \to B)$.

  By generation $\Gamma \vdash \lambda x. f x : A \to B$, so $\Gamma \vdash f : A \to B$, then $\Gamma \vdash {\bf pure \:} f : {\bf K}(A \to B)$ by ${\bf K}_I$.

  \vspace{\baselineskip}

  4) Let $\Gamma \vdash {\bf pure \:} (\langle \pi_1 P, \pi_2 P \rangle) : {\bf K}(A \times B)$.

  By generation $\Gamma \vdash \langle \pi_1 P, \pi_2 P \rangle : A \times B$, then $\Gamma \vdash P : A \times B$.

  By ${\bf K}_I$, $\Gamma \vdash {\bf pure \:} P : {\bf K}(A \times B)$.

  \vspace{\baselineskip}

  5) $\Gamma \vdash {\bf pure \:} ({\bf let \: pure \:} x = M {\: \bf in \: } x) : {\bf K}^2 A$.

  Then $\Gamma \vdash M : {\bf K}^2 A$ and $x : {\bf K} A \vdash x : {\bf K} A$, so $\Gamma \vdash M : {\bf K}^2 A$.

  \vspace{\baselineskip}

  6) Let $\vdash {\bf pure \:} ({\bf let \: pure \:} \underline{\quad} = \underline{\quad} {\: \bf in \:} N) : {\bf K}^2 A$.

  By generation ${\bf let \: pure \:} \underline{\quad} = \underline{\quad} {\: \bf in \:} N : {\bf K}A$, so $\vdash N : A$, then $\vdash {\bf pure \:} N : {\bf K}$.
...
\section{Strong normalization}
  ...
  Strong normalization for ${\bf IK}$ is proved in \cite{Abe} \cite{ModalK}. So we consider simply typed lambda calculus with ${\bf K}_I$ rule and show that $\lambda_{\to, \times} + {\bf K}_I$ is strongly normalizable.
  ...
  \begin{proof}
    ...
    \begin{defin} The set of strongly computable terms for every type $T \in \mathbb{T_{\bf K}}$.
      \begin{itemize}
        \item Let $A \in \mathbb{T}$, then $SC_A = \{ M : A \: | \: \text{$M$ is strongly normalizing}\}$;
        \item $SC_{A \to B} = \{ M : A \to B \: | \: \forall A \in SC_A, M N \in SC_B \}$;
        \item $SC_{A_1 \times A_2} = \{ M : A \times B \: | \: \pi_i M \in SC_{A_i}, i \in \{ 1, 2 \} \}$;
        \item $SC_{{\bf K}A} = \{ {\bf pure \:} M : {\bf K}A \: | \: M \in SC_A \}$
      \end{itemize}
    \end{defin}
    ...
    \begin{lemma}
      ...
      ii) If $M \rightarrow_{\beta} M^{'}$ and $M \in SC_A$, then $M^{'}$;
    ...
    \begin{proof}
      ...
      Let us consider case with $SC_{{\bf K}A}$.
      If ${\bf pure \:} M \in SC_{{\bf K}A}$, then $M \in SC_A$ and $M$ is strongly normalizible.
      So ${\bf pure \:} M$ is strongly normalizible, otherwise there would be an infinite reduction path in ${\bf pure \:} M$.
      ...
      Let ${\bf pure \:} M \rightarrow_{\beta} {\bf pure \:} M^{'}$ and ${\bf pure \:} M \in SN_{{\bf K}A}$.
      By assumption, $M \in SN_A$ and $M \rightarrow_{\beta} M^{'}$, so $M^{'} \in SN_A$. Hence ${\bf pure \:} M^{'} \in SC_{{\bf K}A}$ by the first statement of the lemma.
      ...
      ${\bf pure \:} M^{'}$ is a neutral by the definition. By assumption $M$ is a strongly normalizing. So ${\bf pure\:} M$ is a strongly normalizing by the first part of the current lemma.

      \vspace{\baselineskip}

      iv)

      Let $x_1 : A_1,\dots,x_n : A_n \vdash {\bf pure \:} M : {\bf K}A$ and $\forall i \in \{ 1,\dots,n\}, N_i \in SC_{A_i}$.

      By generation $x_1 : A_1,\dots,x_n : A_n \vdash M : A$ and by assumption $M [\vec{x} := \vec{N}] \in SC_B$.

      Hence, by the first part of lemma, ${\bf pure \:} (M [\vec{x} := \vec{N}]) \in SC_{{\bf K}B}$.
    \end{proof}
