...
\end{prooftree}
  ...
  11) ${\bf pure \:} (\lambda x. f x) \rightarrow_{\eta} {\bf pure \:} f$;

  12) ${\bf pure \:} (\langle \pi_1 P, \pi_2 P \rangle) \rightarrow_{\eta} {\bf pure \:} P$;

  13) ${\bf pure \:} ({\bf let \: pure \:} x = M {\: \bf in \: } x) \rightarrow_{\eta} {\bf pure \:} M$.
  \end{defin}
...
Induction on the derivation of $\Gamma \vdash {\bf pure \: } M : \textbf{K}\alpha$ and $\Gamma \vdash {\bf let \: pure} \: \vec{N} = \vec{M} \: {\bf in} \: M : {\bf K} B$ respectively.
  ...
  1) Let $\Gamma \vdash x : A$, where $\Gamma = \Delta, x : \alpha$, $x \in \mathbb{V}$.
...
1) Let $\Gamma \vdash {\bf let \: pure \:} \langle \vec{x}, y, \vec{z} \rangle = \langle \vec{M}, {\bf let \: pure \: } \vec{w} = \vec{N} {\bf \: in \: } Q, \vec{P} \rangle {\: in \:} R : {\bf K }B$
