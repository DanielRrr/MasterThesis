  ...
  \begin{minipage}{0.5\textwidth}
    \begin{flushleft}
  	...
  	\begin{prooftree}
    	...
    	\UnaryInfC{$\Gamma \vdash \textbf{K}\alpha$}
  \end{prooftree}
    \end{flushleft}
  \end{minipage}
  \begin{minipage}{0.5\textwidth}
    \begin{flushright}
  	...
  	\begin{prooftree}
  	\AxiomC{ $\Gamma \vdash \alpha_1 \land \alpha_2$ }
  	\RightLabel{$\land_e, i \in \{ 1, 2 \}$}
  	\UnaryInfC{$\Gamma \vdash \alpha_i$}
  	\end{prooftree}

  	\begin{prooftree}
    	\AxiomC{$\Gamma \vdash \textbf{K}(\alpha \to \beta)$}
    	\AxiomC{$\Gamma \vdash \textbf{K}\alpha$}
    	\RightLabel{$\textbf{K}_{MP}$}
          \BinaryInfC{$\Gamma \vdash \textbf{K}\beta$}
  	\end{prooftree}
    \end{flushright}
  \end{minipage}
  \end{defin}
...
Here are some examples of derivation trees.
  ...
  \begin{prooftree}
  \AxiomC{$x : \alpha \vdash x : \alpha$}
  \RightLabel{$\textbf{K}_I$}
  \UnaryInfC{$x : \alpha \vdash {\bf pure \: } x : \textbf{K} \alpha$}
  \RightLabel{$\rightarrow_i$}
  \UnaryInfC{$\vdash (\lambda x. {\bf pure \: } x) : \alpha \to \textbf{K}\alpha$}
  \end{prooftree}


  \begin{prooftree}
    \AxiomC{$f : \textbf{K}(\alpha \to \beta) \vdash f : \textbf{K}(\alpha \to \beta)$}
    \AxiomC{$x : \textbf{K}\alpha \vdash x : \textbf{K}\alpha$}
    \RightLabel{$\textbf{K}_{app}$}
    \BinaryInfC{$f : \textbf{K}(\alpha \to \beta), x : \textbf{K}\alpha \vdash f \star x :
    \textbf{K}\beta$}
    \RightLabel{$\rightarrow_i$}
    \UnaryInfC{$f : \textbf{K}(\alpha \to \beta) \vdash \lambda x. f \star x : \textbf{K}\alpha \to \textbf{K}\beta$}
    \RightLabel{$\rightarrow_i$}
    \UnaryInfC{$\vdash \lambda f. \lambda x. f \star x: \textbf{K}(\alpha \to \beta) \to
    \textbf{K}\alpha \to \textbf{K}\beta$}
  \end{prooftree}
