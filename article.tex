...
\begin{defin} The set of strongly computable terms:
  \begin{itemize}
    ...
    \item $SC_{{\bf K}(A \to B)} = \{ M : {\bf K}(A \to B) \: | \: \forall f \in SC_{A \to B}, \forall x \in SC_A, \forall N \in SC_{{\bf K}A}, {\bf let \: pure \:} f, x = M, N {\: \bf in \:} f x \in SC_{{\bf K}B} \}$ for $A,B \in \mathbb{T}_{\bf K}$.
  \end{itemize}
\end{defin}
