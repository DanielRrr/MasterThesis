...
\begin{defin} The set of underlined terms.
  ...
  \begin{itemize}
    ...
    \item $\vec{x} \in \mathbb{V}^{*}, \vec{M} \in \underline{\Lambda}^{*}, N \in \underline{\Lambda} \Rightarrow {\bf let \: pure \:} \vec{x} = \vec{M} {\: \bf in \:} N \in \underline{\Lambda}$, where $length(\vec{x}) = length(\vec{M})$;
...
\begin{proof}
  ...
  $\begin{array}{lll}
  & \phi({\bf pure \:} (M [x := N])) = & \\
  &\quad\quad\quad\quad\quad\quad \text{By the definition of $\phi$} & \\
  & {\bf pure \:} (\phi(M [x := N])) = & \\
  &\quad\quad\quad\quad\quad\quad \text{Induction hypothesis}& \\
  & {\bf pure \:} (\phi(M) [x := \phi(N)]) = &\\
  &\quad\quad\quad\quad\quad\quad \text{Substitution definition}& \\
  & ({\bf pure \:} \phi (M)) [x := \phi(N)]&
  \end{array}$
...
Strong functor on a monoidal category is an endofunctor equipped with a natural transformation so called tensorial strength:
...
\item $\tau_{A, B} \circ id_A \otimes \eta_B = \eta_{A \otimes B}$, i.e. the following diagram commutes:
...
&\quad\quad\quad\quad\quad\quad \text{By interpretation}& \\
&\mathcal{K}([\![M]\!]) \circ u_{\mathds{1}} = & \\
&\quad\quad\quad\quad\quad\quad \text{By the definition of an applicative functor}& \\
&\mathcal{K}([\![M]\!]) \circ \eta_{\mathds{1}} = & \\
&\quad\quad\quad\quad\quad\quad \text{By naturality of $\eta$} & \\
&\eta_{[\![A]\!]} \circ [\![M]\!] = &\\
&\quad\quad\quad\quad\quad\quad \text{By interpretation} & \\
...
$\begin{array}{lll}
&\eta_{\hat{B}} \circ f = & \\
&\quad\quad\quad\quad\quad\quad \text{By the definition}&\\
...
&\quad\quad\quad\quad\quad\quad \text{By the definiton}& \\
...
&\quad\quad\quad\quad\quad\quad \text{By composition} & \\
...
&\quad\quad\quad\quad\quad\quad \text{By $\beta$-reduction rules} & \\
...
\begin{proof}
  $ $ The second part is obvious. We will consider only the first statement.
...
\begin{lemma}
  ...
  $\tau_{A, B} \circ id_A \times \eta_B = \eta_{\widehat{A \times B}}$
...
&\quad\quad\quad\quad\quad\quad \text{$\eta$-reduction}& \\
&[p, {\bf \: pure \:} p] =& \\
&\quad\quad\quad\quad\quad\quad \text{The definition of $\eta$}& \\
