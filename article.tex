...
Here are some examples of derivable terms:

1) $\vdash (\lambda x. {\bf pure \: } x) : A \to \textbf{K}A$;

2) $\vdash \lambda f. \lambda x. {\bf let \: pure} \: \langle g, y \rangle = \langle f, x \rangle {\: \bf in \:} g y : {\bf K}(A \to B) \to {\bf K} A \to {\bf K} B$

3) $\lambda f. \lambda x. {\bf let \:pure} \: \langle g, y \rangle = \langle {\bf pure \:} f, x \rangle {\: \bf in \: } g y : (A \to B) \to {\bf K} A \to {\bf K} B$
...
1) ${\bf K} : A \mapsto {\bf K}A$;

2) ${\bf K} : [x,M] \in Hom_{\mathcal{C}(\lambda)}(A,B) \mapsto \text{fmap } f = [y, {\bf let \: pure \:} x = y {\: \bf in \:} M] \in Hom_{\mathcal{C}(\lambda)}({\bf K} A, {\bf K} B)$.

\end{defin}

\begin{lemma} Functoriality

  i) ${\bf K}(g \circ f) = {\bf K}(g) \circ {\bf K}(f)$;
...
\begin{defin} Tensorial strength
  ...
  Let $[p, \langle {\bf pure \:} (\pi_1 p), \pi_2 p \rangle] \in Hom_{\mathcal{C}(\lambda)}(A \times {\bf K}B, {\bf K}A \times {\bf K}B)$.
...
\begin{lemma} Weak commutativity.
  ...
  $\begin{array}{lll}
  & \text{fmap } ([p, \langle \pi_2 p, \pi_1 p \rangle]) \circ \ast_{A, B} \circ [p, \langle {\bf pure \:} (\pi_1 p), \pi_2 p \rangle] = & \\
  & \ast_{B, A} \circ [p^{'}, \langle \pi_1 p^{'}, {\bf pure \:} (\pi_2 p) \rangle] \circ [p, \langle \pi_2 p, \pi_1 p \rangle]&
  \end{array}$
\end{lemma}

\begin{proof}
  Follows from tensorial strength and commutativity of product.
\end{proof}
